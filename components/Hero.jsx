				...
				<div className="container">
					<div className="h-[30rem] text-center flex flex-col justify-center gap-3 lg:mr-[30rem] lg:text-left ease-linear duration-300">
						...
						>
							...
							<button
								type="submit"
								className="w-full lg:w-1/6 h-2/4 lg:h-full flex justify-center items-center text-lg font-medium rounded-md bg-red-600 hover:bg-orange-600 ease-linear duration-300 text-white"
							>
								Search
							</button>
						</form>
						<h6 className="text-gray-500 flex gap-1 items-center">
							...
							<div className="flex gap-1">
								{trending.map((item, index) => (
									<Link
										href={`#categories`}
										className="hover:text-blue-700 ease-linear duration-100"
										key={index}
									>
										{`${item.name}${index !== 3 ? ", " : ""}`}
									</Link>
