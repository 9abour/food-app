...
import { getMeals } from "@/slices/mealsSlice";
	...
	return (
		<div className="my-[5rem]">
			<div className="container">
				<div
					id="categories"
					className="title flex items-center relative p-3 text-start text-2xl font-semibold text-white mb-2 bg-red-600 bg-[url('http://www.ansonika.com/fooyes/img/pattern.svg')]"
				>
				...
				>
					{categories.map(item => (
						<SwiperSlide className="!h-[10rem]" key={item.idCategory}>
							<div
								className="h-full w-full relative shadow-xl cursor-pointer"
								onClick={() => {
									setMeal(item.strCategory);
								}}
							>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-[3rem]">
					{state != 0 &&
						state.meals.map(item => (
							<Link key={item.idMeal} href={`meal/${item.idMeal}`}>
								<div className="flex flex-col relative">
									...
									<span className="absolute bg-red-600 text-yellow-300 shadow-md rounded-[3px] text-md font-medium right-2 top-2 px-2 py-0 z-20">
										8.8
									</span>
